...
@section('content')
    ...
    <div class="detail-shell">
        <div class="detail-header">
            <a class="btn btn--ghost btn--sm text-white/70 hover:text-white" href="{{ url()->previous() !== url()->current() ? url()->previous() : route('products.index') }}">← بازگشت</a>
        ...
        <div class="detail-grid">
            ...
            <div class="detail-col">
                <div class="detail-card panel p-0 bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
                    <div class="p-6 border-b border-white/10 bg-black/10">
                        <div class="h4">{{ ($product->type ?? null) === 'video' ? 'ویدیو و توضیحات' : 'توضیحات' }}</div>
                    </div>
                    ...
                    <div class="p-6 detail-scroll">
                        ...
                        @if (($product->type ?? null) !== 'video')
                            ...
                            @if (($ratingsArePublic ?? false))
                                <div class="h4 mb-3">امتیاز کاربران</div>

                                @if (isset($avgRating) && $avgRating !== null)
                                    <div class="flex items-center gap-4 mb-8 bg-white/5 p-4 rounded-lg border border-white/10">
                            ...
                            @if (($reviewsArePublic ?? false))
                                <div class="h4 mb-3">نظرات کاربران</div>
            ...
            <div class="detail-col">
                <div class="detail-card panel p-0 bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
                    <div class="p-6 border-b border-white/10 bg-black/10">
                        <div class="h4">
                            {{ ($product->type ?? null) === 'note' ? 'محتوا' : 'امتیاز و نظرات' }}
                        </div>
                    </div>
                    ...
                    <div class="p-6 detail-scroll">
                        @if (($product->type ?? null) === 'video')
                            @if (($ratingsArePublic ?? false) || ($reviewsArePublic ?? false))
                                ...
                                @endif
                            @else
                                <div class="text-muted">امتیاز و نظرات برای این محتوا غیرفعال است.</div>
                            @endif
                        @endif
