        ...
        <div class="flex-1 overflow-y-auto custom-scrollbar pl-4">
            ...
            @else
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    <!-- Cart Items -->
                    <div class="lg:col-span-8 stack stack--md">
                        @foreach ($items as $item)
                            <div class="panel p-6 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-sm flex flex-col md:flex-row items-center gap-6 group hover:border-brand/30 transition-all">
                                <!-- Thumbnail -->
                                <div class="w-full md:w-32 h-32 rounded-xl bg-white/10 overflow-hidden flex-shrink-0 relative">
                                ...
                                <div class="flex-1 w-full text-center md:text-right">
                                    <h3 class="text-xl font-bold text-white mb-2">{{ $item->product?->title ?? 'محصول نامشخص' }}</h3>
                                    <div class="text-brand text-lg font-bold mb-4">
                                        {{ number_format($item->unit_price) }} 
                                        <span class="text-sm font-normal text-muted">{{ $currencyUnit ?? 'تومان' }}</span>
                                    </div>
                                    
                                    <form method="post" action="{{ route('cart.items.destroy', $item->id) }}" class="inline-block">
                                        @csrf
                                        @method('delete')
                    ...
                    <div class="lg:col-span-4 sticky top-0">
                        <div class="panel p-8 bg-white/5 border border-white/10 rounded-3xl backdrop-blur-xl shadow-xl">
